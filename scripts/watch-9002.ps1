...
$LogPath = Join-Path $Root "lcc-9002-watchdog.log"
$ApiOut = Join-Path $Root "lcc-api-9002.log"
$ApiErr = Join-Path $Root "lcc-api-9002.err.log"
...
function Start-Api9002 {
  $listener = Get-NetTCPConnection -LocalPort 9002 -ErrorAction SilentlyContinue |
    Where-Object { $_.State -eq "Listen" }
  if ($listener) {
    $pids = @($listener | Select-Object -ExpandProperty OwningProcess -Unique | Where-Object { $_ -gt 0 })
    foreach ($listenerPid in $pids) {
      Write-WatchLog "stopping unhealthy 9002 listener pid=$listenerPid"
      try {
  ...
  Write-WatchLog "starting 9002 API"
  $args = @(
    "-NoProfile",
    "-ExecutionPolicy", "Bypass",
    "-Command",
    "& { `$env:LCC_API_PORT='9002'; `$env:LCC_MAX_ACTIVE_SESSIONS='20'; `$env:CARGO_TARGET_DIR='target-9002'; .\scripts\dev-api.ps1 }"
  )
...
while ($true) {
  if (-not (Test-ApiHealth)) {
    Write-WatchLog "9002 health failed"
    Start-Api9002
